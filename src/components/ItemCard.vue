...
const displayTitle = computed(() => {
  ...
  const isSingleElement =
    props.symbol &&
    !props.atomicId &&
    (!props.components || Object.keys(props.components).length <= 1)
...
<template>
  ...
  <div
    ...
    :class="
      twMerge(
        props.size === 'large' ? 'text-[18px] space-x-2 p-2' : 'text-[15px] space-x-1.5 p-1.5',
        ...
        (props.selected || props.isHovered) &&
          'bg-gradient-to-b from-white to-[#e0f2fe] dark:from-[#262626] dark:to-neutral-700 border-[#999] dark:border-neutral-500 shadow-md',
        props.isHovered && 'scale-[1.03]',
        props.isRejected &&
          'border-red-500 dark:border-red-500 !bg-red-50 dark:!bg-red-900/30 shadow-md animate-shake'
      )
    "
  >
    <span v-if="emoji">
    ...
    <img
      v-else-if="icon"
      :src="icon"
      class="h-[18px] w-auto flex-shrink-0 pointer-events-none dark:invert dark:opacity-[0.871]"
      :alt="title"
      draggable="false"
      @contextmenu.prevent
    />
